...
(function ($) {
  ...
  function mainMenu () {
    ...
    const mobilePanelMenu = $('.mobile-menu-panel .panel-menu')
    const navClose = $('.panel-close')
  ...
  function stickyHeader () {
    ...
    const offsetTop = siteHeader.outerHeight()
    const offsetTopAnimation = offsetTop + 150
  ...
  function headerBreakpointResize () {
    ...
    const navContainer = $('.navbar-wrapper')
    ...
    $('.project-item').on('inview', function (event, isInView) {
      if (isInView) {
        $.each($('.stats-bar'), function () {
          const dataCount = $(this).attr('data-value')
          let progress = $(this).find('.bar-line')
